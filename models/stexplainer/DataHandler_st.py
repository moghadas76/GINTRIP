...
class DataHandler:
    ...
    def load_st_dataset(self, dataset):
        # output B, N, D
        if dataset == 'PEMS4':
            data_path = os.path.join('../../datasets/PEMS04/PEMS04.npz')
            data = np.load(data_path)['data'][:, :, 0]
            start_date = '2018-01-01 00:00:00'
            time = pd.date_range(start_date, periods=data.shape[0], freq = '5MIN')
            
        elif dataset == 'PEMS8':
            data_path = os.path.join('../../datasets/PEMS08/PEMS08.npz')
            data = np.load(data_path)['data'][:, :, 0]
            start_date = '2016-07-01 00:00:00'
            time = pd.date_range(start_date, periods=data.shape[0], freq = '5MIN')
        elif dataset == 'PEMS3':
            data_path = os.path.join('../../datasets/PEMS03/PEMS03.npz')
            data = np.load(data_path)['data'][:, :, 0]
            start_date = '2018-09-01 00:00:00'
            time = pd.date_range(start_date, periods=data.shape[0], freq = '5MIN')
        elif dataset == 'PEMS7':
            data_path = os.path.join('../../datasets/PEMS07/PEMS07.npz')
            data = np.load(data_path)['data'][:, :, 0]
