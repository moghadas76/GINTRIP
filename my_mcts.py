...
from gib_utils import PlotUtils
import random
import pdb
...
def mcts(data, gnnNet, prototype, index=None):
    """
    Monte Carlo Tree Search (MCTS) algorithm for finding the best coalition in a graph.

    Args:
        data (torch_geometric.data.Data): The input graph data.
        gnnNet (torch.nn.Module): The graph neural network model.
        prototype (torch.Tensor): The prototype tensor.
        index (int): The index of the graph.

    Returns:
        tuple: A tuple containing the best coalition, its probability, and the corresponding embedding.
    ...
    #TODO: num_node, device fix
    score_func = partial(gnn_prot_score, data=data, gnnNet=gnnNet, prototype=prototype, num_nodes=num_nodes, device=data.x.device, idx=index)
...
def gnn_prot_score(coalition, data, gnnNet, prototype, num_nodes, device, idx):
    ...
    mask_data = Batch.from_data_list([mask_data])
    try:
        _, _, emb, _, _, _, _, _, _, _, _, _ = gnnNet(mask_data.x, idx)
